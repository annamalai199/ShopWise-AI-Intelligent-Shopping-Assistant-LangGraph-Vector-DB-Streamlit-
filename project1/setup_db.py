"""
ShopWise AI - Database Setup Module

Initializes the SQLite database with tables and seed data for the shopping platform.
Creates three main tables: products, reviews, and orders with comprehensive sample data.

Database Schema:
    - products: Product catalog with pricing and organic certification
    - reviews: Customer reviews with ratings and text feedback
    - orders: Order history with product references and timestamps
...
def create_database():
    """
    Create and populate the ShopWise AI database with tables and seed data.
    
    This function will:
    1. Create the SQLite database file if it doesn't exist
    2. Create all required tables (products, reviews, orders)
    3. Populate tables with comprehensive seed data
    4. Display creation summary
    """
    try:
        print("=" * 70)
        print("ShopWise AI - Database Setup")
        print("=" * 70)
        print()
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS search_history (
                ...
                FOREIGN KEY (ordered_product_id) REFERENCES products(id)
            )
        """)
        ...
        print("   ✓ Orders table created (with user_id)")
        ...
        print("🛍️  Inserting product catalog...")
        products = [
        # --- Honey (8) ---
        ...
        organic_count = sum(1 for p in products if p[5] == 1)
        print(f"   ✓ {len(products)} products added")
        print(f"   ✓ {organic_count} organic products ({organic_count/len(products)*100:.0f}%)")
        print()
